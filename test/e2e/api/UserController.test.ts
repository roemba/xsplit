...
test('create user', async() => {
    const username = "mcTest";
    const secret = ("ss4cbx9U3QmgDjDkf14hycjydbfFF");
    ...
    expect(createdUser.username).toBe("mcTest");
...
test('get me', async() => {
    ...
    const derivationResult = deriveKeypair(process.env.TEST_SECRET);

    expect(currentUser.username).toBe("mcTest");
